...
// waitForNode returns once qtumd is ready to serve
// block queries.
...
// Prune attempts to prune blocks in qtumd every
// pruneFrequency.
func (i *Indexer) Prune(ctx context.Context) error {
	...
	for {
		...
		case <-tc.C:
			...
			logger.Infow("attempting to prune qtumd", "prune height", pruneHeight)
			prunedHeight, err := i.client.PruneBlockchain(ctx, pruneHeight)
			if err != nil {
				logger.Warnw(
					"unable to prune qtumd",
					"prune height", pruneHeight,
					"error", err,
				)
			} else {
				logger.Infow("pruned qtumd", "prune height", prunedHeight)
			}
		}
	}
}
