...
build:
	docker build -t rosetta-bitcoin:latest https://github.com/qtumproject/rosetta-qtum.git
...
run-testnet-online:
	docker run --rm --ulimit "nofile=${NOFILE}:${NOFILE}" -v "${PWD}/bitcoin-data:/data" -e "MODE=ONLINE" -e "NETWORK=TESTNET" -e "PORT=8080" -p 8080:8080 -p 18333:18333 rosetta-bitcoin:latest
